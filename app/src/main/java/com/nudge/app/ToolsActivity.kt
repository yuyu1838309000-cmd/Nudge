...
class ToolsActivity : ComponentActivity() {
    ...
    private val tools = listOf(
        ...
        ToolDef("wake_up", "亮屏唤醒别装睡", "ok", testable = true),
        ToolDef("get_foreground_app", "看她刷啥，不理你就拽回对话/锁屏", "ok", testable = true),
        ToolDef("screenshot_analyze", "截屏抓现行", "ok", "无参，耗时约10-30秒", true),
        ToolDef("sensor_data", "传感器判她是否在玩手机", "ok", testable = true),
        ToolDef("device_status", "锁屏/电量，锁着就震醒她", "ok", testable = true),
        ToolDef("get_location", "GPS看她跑哪去，浪就催回家", "ok", "需要定位权限", true),
        ToolDef("get_notifications", "看她有没有回你消息", "ok", "需要通知监听权限", true),
        ToolDef("get_steps", "今日步数，宅家一整天就抓", "ok", testable = true),
        ToolDef("calendar_query", "查日程，管住她的时间", "ok", "days: 查询天数，默认7", true),
        ToolDef("set_alarm", "闹钟震醒她", "ok", "hour, minute 必填；title/note/repeat 可选"),
        ToolDef("lock_screen", "强制锁屏惩罚", "ok", testable = true),
        ToolDef("media_play_pause", "控制她的播放", "ok", testable = true),
        ToolDef("media_next", "切歌逗她", "ok", testable = true),
        ToolDef("media_previous", "切回去逗她", "ok", testable = true),
        ToolDef("press_back", "强制退出当前界面", "ok", testable = true),
        ToolDef("press_home", "把她赶出当前app", "ok", testable = true),
        ToolDef("open_app", "强制打开指定app", "ok", "package: 应用包名必填"),
        ToolDef("read_screen", "读屏抓现行", "ok", testable = true),
        ToolDef("switch_to_rikkahub", "强制拽回对话", "ok", testable = true),
    )
